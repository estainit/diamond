...
pub mod vote_counting_methods {
    ...
    // Plurality
    pub const PluralityLog: &str = "PluralityLog";
    // Plurality logarythmic and (x )minutes for Y/N/A (x+1/2) Minutes for N/A
    pub const TwoRRo: &str = "TwoRRo";
    //"Two-Round Runoff"
    pub const InstantRo: &str = "InstantRo";
    //"Instant Runoff"
    pub const BordaCount: &str = "BordaCount"; //"Borda Count"
}
...
pub fn autoCreatePollingForProposal(params: &mut JSonObject, block: &Block) -> bool
{
    ...
    let custom_stringified = stringifyAutoGeneratedProposalPolling(&mut params);
    let dExtHash = ccrypto::keccak256(&custom_stringified);
    dlog(
        &format!("custom Stringified Auto-generated polling (dExtHash: {}): {}", dExtHash, custom_stringified),
        constants::Modules::App,
        constants::SecLevel::Trace);
    params["dExtHash"] = dExtHash.into();

    let dLen = cutils::padding_length_value(
        (custom_stringified.len() + dExtHash.to_string().len()).to_string(),
        constants::LEN_PROP_LENGTH);
    params["dLen"] = dLen.into();

    let mut doc: Document = load_document(params, block, 0);
    ...
    let abs_no_timeframe_by_minutes: TimeByMinutesT = (voting_timeframe * 60.0 * 1.5) as u64; // 90 = 60 minute + 30 minute => 1.5 * yesTime
    dlog(
        &format!("abs and no timeframe by minutes = {}", abs_no_timeframe_by_minutes),
        constants::Modules::App,
        constants::SecLevel::Trace);

    let end_date: CDateT = cutils::minutes_after(
        abs_no_timeframe_by_minutes,
        params["startDate"].to_string());

    let ZERO = "0";
    let values: HashMap<&str, &str> = HashMap::from([
        ("pll_creator", &*params["dCreator"].to_string()),
        ("pll_type", &*params["dType"].to_string()),
        ("pll_class", &*params["dClass"].to_string()),
        ("pll_ref", &*params["dRef"].to_string()),  // referenced voting subject(could be proposalHash,..
        ("pll_ref_type", &*params["dRefType"].to_string()), // referenced voting subject(could be proposalHash,..
        ("pll_ref_class", &*params["dRefClass"].to_string()),
        ("pll_start_date", &*params["startDate"].to_string()),
        ("pll_end_date", &*end_date), // end date is the deadline for absatin/no voting
        ("pll_timeframe", &voting_timeframe.to_string()),
        ("pll_version", &*params["dVer"].to_string()),
        ("pll_comment", &*params["dComment"].to_string()),
        ("pll_y_count", ZERO),
        ("pll_y_shares", ZERO),
        ("pll_y_gain", ZERO),
        ("pll_y_value", ZERO),
        ...
        ("pll_a_gain", ZERO),
        ("pll_a_value", ZERO),
        ("pll_status", &*params["status"].to_string()),
        ("pll_hash", &*pll_hash)]);
    dlog(
        &format!("Insert an auto-generated polling for proposal values: {}", dump_hashmap_of_str(&values)),
        constants::Modules::App,
        constants::SecLevel::Trace);
...
pub fn stringifyAutoGeneratedProposalPolling(params: &mut JSonObject) -> String
{
...
pub fn initPollingProfiles()
{
    ...
    let values: HashMap<&str, &str> = HashMap::from([
        (
            "ppr_name",
            "Basic"//&*POLLING_PROFILE_CLASSES[a_polling_profile_key]["ppName"].to_string()
        ),
        ("ppr_activated", constants::YES),//&*POLLING_PROFILE_CLASSES[a_polling_profile_key]["activated"].to_string()),
        ("ppr_perform_type", poll_performance_types::Transparent),//&*POLLING_PROFILE_CLASSES[a_polling_profile_key]["performType"].to_string()),
        ("ppr_amendment_allowed", constants::NO),//&*POLLING_PROFILE_CLASSES[a_polling_profile_key]["voteAmendmentAllowed"].to_string()),
        ("ppr_votes_counting_method", vote_counting_methods::Plurality),//&*POLLING_PROFILE_CLASSES[a_polling_profile_key]["votesCountingMethod"].to_string()),
        ("ppr_version", "0.0.8"),
    ]);
    q_insert(
        STBL_POLLING_PROFILES,
