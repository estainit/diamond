...
pub fn autoCreatePollingForProposal(params: &mut JSonObject, block: &Block) -> bool
{
    ...
    let custom_stringified = stringifyAutoGeneratedProposalPolling(params);
    ...
    params["dExtHash"] = dExtHash.clone().into();
    ...
    let values: HashMap<&str, &str> = HashMap::from([
        ...
        ("pll_class", pll_class.as_str()),
        ("pll_ref", pll_ref.as_str()),  // referenced voting subject(could be proposalHash,..
        ("pll_ref_type", pll_ref_type.as_str()), // referenced voting subject(could be proposalHash,..
        ("pll_ref_class", pll_ref_class.as_str()),
        ("pll_start_date", pll_start_date.as_str()),
        ("pll_end_date", &pll_end_date.as_str()), // end date is the deadline for Absatin/No voting
        ("pll_timeframe", pll_timeframe.as_str()),
        ("pll_version", pll_version.as_str()),
        ("pll_comment", pll_comment.as_str()),
        ("pll_y_count", &ZERO),
        ("pll_y_shares", &ZERO),
        ("pll_y_gain", &ZERO),
        ("pll_y_value", &ZERO),
        ...
        ("pll_a_gain", &ZERO),
        ("pll_a_value", &ZERO),
        ("pll_status", pll_status.as_str()),
        ("pll_hash", &pll_hash.as_str())
    ]);
    dlog(
...
pub fn stringifyAutoGeneratedProposalPolling(params: &JSonObject) -> String
{
