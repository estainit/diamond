...
bool PollingHandler::autoCreatePollingForProposal(QJsonObject& params)
{
  ...
  if (VersionHandler::isOlderThan(params.value("dVer").to_string(), "0.0.8") == 1)
    params["dVer"] = "0.0.0";
  ...
  CDateT end_date = CUtils::minutesAfter(
    ...
    params.value("startDate").to_string());

  uint32_t ZERO = 0;
  QVDicT values {
    {"pll_creator", params.value("dCreator").to_string()},
    {"pll_type", params.value("dType").to_string()},
    {"pll_class", params.value("dClass").to_string()},
    {"pll_ref", params.value("dRef").to_string()},  // referenced voting subject(could be proposalHash,..
    {"pll_ref_type", params.value("dRefType").to_string()}, // referenced voting subject(could be proposalHash,..
    {"pll_ref_class", params.value("dRefClass").to_string()},
    {"pll_start_date", params.value("startDate").to_string()},
    {"pll_end_date", end_date}, // end date is the deadline for absatin/no voting
    {"pll_timeframe", QVariant::fromValue(voting_timeframe)},
    {"pll_version", params.value("dVer").to_string()},
    {"pll_comment", params.value("dComment").to_string()},
    {"pll_y_count", ZERO},
    {"pll_y_shares", ZERO},
    {"pll_y_gain", ZERO},
    {"pll_y_value", ZERO},
    ...
    {"pll_a_gain", ZERO},
    {"pll_a_value", ZERO},
    {"pll_status", params.value("status").to_string()},
    {"pll_hash", pll_hash}
  };
...
QString PollingHandler::stringifyAutoGeneratedProposalPolling(QJsonObject& params)
{
  ...
  QString out = "{";
  out += "\"dCDate\":\"" + params.value("startDate").to_string() + "\",";
  out += "\"dComment\":\"" + params.value("dComment").to_string() + "\",";
  out += "\"dClass\":\"" + params.value("dClass").to_string() + "\",";  // CConsts.POLLING_PROFILE_CLASSES.Basic.ppName
  out += "\"dRef\":\"" + params.value("dRef").to_string() + "\",";
  out += "\"dRefClass\":\"" + params.value("dRefClass").to_string() + "\",";
  out += "\"dRefType\":\"" + params.value("dRefType").to_string() + "\",";
  out += "\"dType\":\"" + params.value("dType").to_string() + "\",";
  out += "\"dVer\":\"" + params.value("dVer").to_string() + "\",";
  out += "\"dCreator\":\"" + params.value("dCreator").to_string() + "\",";
...
//  QString rrr = PollingHandler::POLLING_PROFILE_CLASSES.value("Basic").value("zzz").to_string();
//  auto sss3 = PollingHandler::POLLING_PROFILE_CLASSES["Basic"]["ppName"].to_string();
//  auto sy3 = PollingHandler::POLLING_PROFILE_CLASSES["Basic"]["voteAmendmentAllowed"].to_string();
//  auto sss = PollingHandler::POLLING_PROFILE_CLASSES["ZKP"]["ppName"].to_string();
//  auto ss2 = PollingHandler::POLLING_PROFILE_CLASSES["ZKP"]["votesCountingMethod"].to_string();


  QueryRes dbl = DbModel::select(
    PollingHandler::stbl_polling_profiles,
    {"ppr_name"},
    {ModelClause ("ppr_name", PollingHandler::POLLING_PROFILE_CLASSES["Basic"]["ppName"].to_string())}
  );
  ...
  for (QString a_polling_profile_key : PollingHandler::POLLING_PROFILE_CLASSES.keys())
  {
    DbModel::insert(
      ...
      QVDicT
      {
        {"ppr_name", PollingHandler::POLLING_PROFILE_CLASSES[a_polling_profile_key]["ppName"].to_string()},
        {"ppr_activated", PollingHandler::POLLING_PROFILE_CLASSES[a_polling_profile_key]["activated"].to_string()},
        {"ppr_perform_type", PollingHandler::POLLING_PROFILE_CLASSES[a_polling_profile_key]["performType"].to_string()},
        {"ppr_amendment_allowed", PollingHandler::POLLING_PROFILE_CLASSES[a_polling_profile_key]["voteAmendmentAllowed"].to_string()},
        {"ppr_votes_counting_method", PollingHandler::POLLING_PROFILE_CLASSES[a_polling_profile_key]["votesCountingMethod"].to_string()},
        {"ppr_version", "0.0.8"},
      }
    );
  }
...
bool PollingHandler::maybeUpdatePollingStat(const CDocHashT& polling_hash)
{
  ...
  vStatistics.m_polling_status = (polling.value("pll_end_date").to_string() < CUtils::getNow()) ? CConsts::CLOSE : CConsts::OPEN;
  ...
  const CDateT& approve_date)
{
  ...
  */
  if (polling.value("pll_ref_type").to_string() == CConsts::POLLING_REF_TYPE::Proposal)
  {
    ProposalHandler::transformApprovedProposalToDNAShares(
      polling,
      approve_date);

  }else if (polling.value("pll_ref_type").to_string() == CConsts::POLLING_REF_TYPE::AdmPolling) {
...
void PollingHandler::treatPollingMissed(
  ...
  const CDateT& approve_date)
{
  QString pll_ref_type = polling.value("pll_ref_type").to_string();
    ...
    force_to_update: bool) -> bool
{
    /*
      ...
      if (force_to_update)
      {
        ...
        clauses.push({"pll_ct_done", CConsts::NO});
      }
...
void PollingHandler::maybeUpdateOpenPollingsStat()
{
   ...
   */
  if (!CMachine::isInSyncProcess())
    clauses.push({"pll_status", CConsts::OPEN});
