...
impl<'m> CMachine<'m> {
       ...
       fn get_block_invoke_gap(&self) -> TimeBySecT
        ...
        TimeBySecT CMachine::getAcceptableBlocksGap()
        {
          ...
          if (constants::TIME_GAIN == 1)
          {
            ...
            // devel
            gapByMinutes = is_in_sync_process() ? (uint32_t)(constants::TIME_GAIN / 0.15) : (uint32_t)(constants::TIME_GAIN / 0.5);
          }
    ...
    */
    ...
    pub fn getInvokeLeavesGap(&self) -> TimeBySecT
    {
        //      return 500;
        let mut gap_by_seconds: TimeBySecT;
        if constants::TIME_GAIN == 1 {
            ...
            if self.is_in_sync_process(false) {
                gap_by_seconds = ((constants::TIME_GAIN * 60) / 9) as TimeBySecT;  // every 17 second
            } else {
                gap_by_seconds = ((constants::TIME_GAIN * 60) / 3) as TimeBySecT;
            }
        }
        return gap_by_seconds;
    }
    ...
    /*
        ...
        //  return 900;
          if (constants::TIME_GAIN == 1)
          {
            ...
            // test ambient
            if is_in_sync_process()              return constants::TIME_GAIN / 1; // it is testing ambianet value
