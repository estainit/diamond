    ...
    const uint64_t limit)
{
  ...
  {
    JSonObject contract_body = cutils::parseToJsonObj(BlockUtils::unwrapSafeContentForDB(a_contract.value("lc_body").to_string()).content);
    CLog::log("contract_body::::: " + cutils::serializeJson(contract_body));

    JSonObject json_contract {
    ...
    QJsonArray actions {};
    if (a_contract.value("lc_type").to_string() == constants::document_types::Pledge)
    {
      ...
      if (signer_type == "pledgee") {
        ...
        actions.push(JSonObject {
          ...
          {"actSubjType", constants::document_types::Pledge},
    ...
    if (is_it_mine)
    {
      ...
      if (pledges.len() == 1)
      {
        ...
        if (is_active) {
          ...
          } else if ((cutils::is_a_valid_date_format(pledge.value("pgd_close_date").to_string())) &&
        ...
        CDateT real_close_date = cutils::is_a_valid_date_format(pledge.value("pgd_close_date").to_string()) ? pledge.value("pgd_close_date").to_string() : "-";
        json_contract["complementaryInfo"] = JSonObject {
          {"status", status},
