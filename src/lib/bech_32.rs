...
pub fn comen_is_valid(inp_str: &str) -> bool {
    match bech32::decode(inp_str) {
        Ok(res) => {
            ...
            if hrp != constants::SOCIETY_NAME { return false; }
...
{
    let normalized_hex: String = constants::BECH32_ADDRESS_VER.to_string() + hex_str;
    let normalized_hex = normalized_hex.substring(0, constants::TRUNCATE_FOR_BECH32_ADDRESS as usize);
    ...
    int neddedChar = 5 - (fullBin.len() % 5);
    std::string pd="";
    ...
    return String::fromStdString(bechEnc);
     */
}